"""Add segment JSON columns to duas table

Revision ID: c3b91166a2db
Revises: 357d48845716
Create Date: 2025-11-12 09:31:11.582977
...
revision: str = 'c3b91166a2db'
down_revision: Union[str, Sequence[str], None] = '357d48845716'
...
def upgrade() -> None:
    ...
    # --- Add new columns to duas ---
    op.add_column('duas', sa.Column('transliteration_segments_json', sa.JSON(), nullable=True))
    op.add_column('duas', sa.Column('translation_segments_json', sa.JSON(), nullable=True))
...
def downgrade() -> None:
    """Downgrade schema."""
    # --- Remove new columns ---
    op.drop_column('duas', 'translation_segments_json')
    op.drop_column('duas', 'transliteration_segments_json')
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('email', name=op.f('admin_users_email_key'))
    )
